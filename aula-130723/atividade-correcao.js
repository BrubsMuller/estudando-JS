//Cie uma função que calcule o valor médio de três números, considerando que esses teram pesos 2, 3 e 4 respectivamente.

function mediaPonderada(n1, n2, n3){
    return ((n1*2 + n2*3 + n3*4) / 9);
}
console.log("A média ponderada entre 6, 7, 8 é " + mediaPonderada(6, 7, 8));
...
console.log("A frase 'Olá, Mundo!' revertida é " + reverterTexto); */

function velocidadeMedia(comprimento, tempo) {
    return (comprimento / tempo);
}
console.log("A velocidade média de uma bicicleta que percorreu 30km em 2h é " + velocidadeMedia(30, 2) + "km/h");
...
// Crie uam função que receba um numero e uma outra função e execute essa de acordo com o umero informado.
function repetir(callback, interation) {
    for(let i = 0; i < interation; i++) {
        callback(i);
    }
}
repetir((index => console.log("Repetição #"+(index+1)) ))